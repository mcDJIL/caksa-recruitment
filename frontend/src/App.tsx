...
const subDivisionOptions: Record<
  Exclude<DivisionFilter, "ALL">,
...
const API_BASE = (
  import.meta.env.VITE_RECRUITMENT_API_URL ?? "http://localhost:3000/api"
).replace(/\/$/, "");
...
const fetchApplications = async (
  ...
  if (!response.ok)
    throw new Error(result.error ?? "Gagal memuat data pendaftar");
  return {
...
const fetchStatusCounts = async (
  ...
  const response = await fetch(
    buildSummaryUrl(query, divisionFilter, subDivisionFilter),
    { credentials: "include" },
  );
  const result = (await response.json()) as ApplicationSummaryResponse;
  if (!response.ok)
    throw new Error(result.error ?? "Gagal memuat ringkasan pendaftar");
...
function App() {
  ...
  const [statusCounts, setStatusCounts] =
    useState<StatusCounts>(emptyStatusCounts);
  ...
  const scoreCriteria: {
    key: keyof Omit<ApplicationScore, "total">;
    label: string;
    max: number;
  }[] = [
    ...
    {
      key: "skills_experience_achievements",
      label: "Skill, pengalaman & prestasi relevan",
      max: 20,
    },
  ...
  const loadApplications = async (targetPage = pagination.page) => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal memuat data pendaftar",
      );
      throw error;
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch(
        buildExportUrl(query, statusFilter, divisionFilter, subDivisionFilter),
        { credentials: "include" },
      );
      if (!response.ok) {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal mengekspor spreadsheet",
      );
    } finally {
  ...
  const openApplication = (application: RecruitmentApplication) => {
    ...
    setScoreDraft({
      ...
      skills_experience_achievements:
  ...
  const handleScoreUpdate = async () => {
    if (
      ...
      !selectedApplication ||
      !scoreDraft ||
      scoreCriteria.some(({ key }) => scoreDraft[key] === null)
    )
      return;
    ...
    try {
      const response = await fetch(
        `${API_BASE}/applications/${encodeURIComponent(selectedApplication.nrp)}/score`,
        {
          method: "PATCH",
          credentials: "include",
      ...
      if (!response.ok)
        throw new Error(result.error ?? "Gagal menyimpan penilaian");
      const updated = { ...selectedApplication, ...result };
      setSelectedApplication(updated);
      setScoreDraft({
        idea: result.idea_score ?? null,
        relevance: result.relevance_score ?? null,
        skills_experience_achievements:
      ...
      setApplications((items) =>
        items.map((item) => (item.nrp === updated.nrp ? updated : item)),
      );
  ...
  const handleAcceptancePriorityUpdate = async (
    acceptancePriority: AcceptancePriority | null,
  ) => {
    if (
      !isAuthenticated ||
      !selectedApplication ||
      selectedApplication.division_code !== "electrical"
    )
      return;
    ...
    try {
      const response = await fetch(
        `${API_BASE}/applications/${encodeURIComponent(selectedApplication.nrp)}/acceptance-priority`,
        {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok)
        throw new Error(result.error ?? "Gagal menyimpan prioritas penerimaan");
      const updated = {
        ...selectedApplication,
        acceptance_priority: result.acceptance_priority ?? null,
        updated_at: result.updated_at ?? selectedApplication.updated_at,
      };
      setSelectedApplication(updated);
      setApplications((items) =>
        items.map((item) => (item.nrp === updated.nrp ? updated : item)),
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Gagal menyimpan prioritas penerimaan",
      );
    } finally {
  ...
  const handleStatusUpdate = async (nrp: string, status: ApplicationStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/applications/${encodeURIComponent(nrp)}/status`,
        {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      const result = (await response.json()) as {
        nrp?: string;
        draft_status?: ApplicationStatus;
        error?: string;
      };
      if (!response.ok || result.nrp !== nrp || !result.draft_status)
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal menyimpan keputusan",
      );
    } finally {
      setUpdatingNrp("");
    }
  ...
  const handlePublishStatuses = async () => {
    if (
      !isAuthenticated ||
      !window.confirm(
        "Terbitkan seluruh keputusan yang sudah disiapkan? Status peserta akan diperbarui sekaligus dan tidak dapat dibatalkan dari sini.",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `${API_BASE}/applications/publish-statuses`,
      ...
      if (!response.ok)
        throw new Error(result.error ?? "Gagal menerbitkan keputusan");
      setNoticeMessage(
        `${result.published ?? 0} keputusan berhasil diterbitkan serentak.`,
      );
      await loadApplications(pagination.page);
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal menerbitkan keputusan",
      );
    } finally {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      setNoticeMessage(
        `${result.sent ?? 0} email berhasil dikirim serentak.`,
      );
    } catch (error) {
      setErrorMessage(
  ...
  ): ApplicationStatus => application.draft_status ?? application.status;

  if (isBooting) {
    return (
      <main className="boot-screen">
        <span className="pulse-dot" />
        Menyiapkan ruang operasi CAKSA
      </main>
    );
  }
        ...
        <section className="login-shell">
          <aside className="login-intro">
            ...
            <div className="intro-copy">
              ...
              <p>
                Ruang kendali untuk memantau setiap kandidat dan menggerakkan
                proses rekrutmen CAKSA dengan presisi.
              </p>
            </div>
          ...
          <form onSubmit={handleLogin} className="login-form">
            ...
            <p className="form-lead">
              Gunakan token admin untuk mengakses data seleksi.
            </p>
            ...
            >
              {isLoggingIn ? "Memeriksa akses…" : "Buka dashboard"}
              <span aria-hidden="true">↗</span>
            </button>
            <p className="security-note">
              Token hanya digunakan untuk membuat sesi aman dan tidak disimpan
              di browser.
            </p>
          </form>
        </section>
      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <div className="topbar-meta">
            ...
            <span className="desktop-only">Recruitment 2025</span>
            <button
      ...
      <div className="dashboard-shell">
        <section className="page-heading">
          <div>
            ...
            <h1>Ruang seleksi.</h1>
            <p>
              Kelola perjalanan kandidat CAKSA dari satu panel yang ringkas dan
              terarah.
            </p>
          </div>
          <div className="heading-actions">
            ...
            <button
              ...
              type="button"
              onClick={() => void handlePublishStatuses()}
              disabled={isPublishing || isRefreshing}
            >
              {isPublishing ? "Menerbitkan…" : "Terbitkan keputusan"}
            </button>
            <button
            ...
            <button
              ...
              disabled={isExporting || isRefreshing || isPublishing}
            >
        ...
        <section className="metrics" aria-label="Ringkasan pendaftar">
          ...
          {statusOptions.map((status) => (
            <article
              className={`metric-card ${statusTone[status]}`}
              key={status}
            >
              <p>{statusLabel[status]}</p>
              <strong>{statusCounts[status]}</strong>
              <span>
                {status === "MEMBER" ? "kandidat terpilih" : "seluruh kandidat"}
              </span>
            </article>
          ))}
        </section>
        ...
        <section className="data-panel">
            ...
            <div className="filters">
              <label className="search-field">
                ...
                <input
                  ...
                  placeholder="Cari nama, NRP, email, atau prodi"
                />
              </label>
              <select
                value={statusFilter}
              ...
              <select
                ...
                onChange={(event) => {
                  setPagination((item) => ({ ...item, page: 1 }));
                  setDivisionFilter(event.target.value as DivisionFilter);
                  setSubDivisionFilter("ALL");
                }}
              ...
              >
                ...
                {divisionFilter !== "ALL" &&
                  subDivisionOptions[divisionFilter].map((division) => (
                    <option key={division.value} value={division.value}>
                      {division.label}
                    </option>
                  ))}
          ...
          {errorMessage && (
            <p className="alert panel-alert" role="alert">
              {errorMessage}
            </p>
          )}
          {noticeMessage && (
            <p className="notice panel-alert" role="status">
              {noticeMessage}
            </p>
          )}
            ...
            <table>
              ...
              <tbody>
                {applications.map((application) => (
                  ...
                  >
                    <td className="nrp">{application.nrp}</td>
                    <td>
                      <strong>{application.full_name}</strong>
                      <span>{application.email}</span>
                    </td>
                    <td>
                      <strong>{application.degree_level_code}</strong>
                      <span>
                        {application.study_program_code} ·{" "}
                        {application.batch_year}
                      </span>
                    </td>
                    <td>
                      <strong>{application.interested_wing_code}</strong>
                      <span>{application.division_code}</span>
                    </td>
                    <td>
                      <span className="file-count">
                        {application.file_metadata?.length ?? 0} berkas
                      </span>
                    </td>
                    <td>
                      <select
                        className={`status-select ${statusTone[activeStatus(application)]}`}
                        value={activeStatus(application)}
                        onChange={(event) => {
                      ...
                      {updatingNrp === application.nrp && (
                        <small>Memperbarui…</small>
                      )}
                    </td>
                    <td>
                      {application.division_code === "electrical"
                        ? application.acceptance_priority
                          ? acceptancePriorityLabel[
                              application.acceptance_priority
                            ]
                          : "Belum diatur"
                        : "—"}
          ...
          <footer className="panel-footer">
            <p>
              Menampilkan <b>{applications.length}</b> dari{" "}
              <b>{pagination.total}</b> kandidat
            </p>
            <div className="pagination">
              <button
                ...
                onClick={() =>
                  setPagination((item) => ({ ...item, page: item.page - 1 }))
                }
              >
                Sebelumnya
              </button>
          ...
          >
            ...
            <div className="detail-content">
              <section className="detail-status">
                ...
                <select
                  id="detail-status"
                  className={`status-select ${statusTone[activeStatus(selectedApplication)]}`}
                  value={activeStatus(selectedApplication)}
                  onChange={(event) =>
                    void handleStatusUpdate(
                      selectedApplication.nrp,
                      event.target.value as ApplicationStatus,
                    )
                  }
                ...
                {selectedApplication.draft_status && (
                  <p className="draft-label">
                    Tersimpan sebagai draft dan belum terlihat oleh peserta.
                  </p>
                )}
              </section>
              {selectedApplication.division_code === "electrical" && (
                <section className="detail-status">
                  <label htmlFor="acceptance-priority">
                    Prioritas penerimaan
                  </label>
                  <select
                    id="acceptance-priority"
                    className="status-select"
                    value={selectedApplication.acceptance_priority ?? ""}
                    onChange={(event) =>
                      void handleAcceptancePriorityUpdate(
                        event.target.value === ""
                          ? null
                          : (event.target.value as AcceptancePriority),
                      )
                    }
                    disabled={isSavingPriority}
                  >
                    <option value="">Belum diatur</option>
                    {Object.entries(acceptancePriorityLabel).map(
                      ([value, label]) => (
              ...
              <div className="detail-grid">
                ...
                <section>
                  ...
                  <p>
                    {selectedApplication.study_program_code} ·{" "}
                    {selectedApplication.batch_year}
                  </p>
                </section>
                <section>
                  <h3>Penempatan</h3>
                  <p>Divisi · {selectedApplication.interested_wing_code}</p>
                  <p>Subivisi · {selectedApplication.division_code}</p>
                </section>
                <section>
                  <h3>Waktu</h3>
                  <p>
              ...
              <section className="document-list">
                <h3>
                  Dokumen{" "}
                  <span>{selectedApplication.file_metadata?.length ?? 0}</span>
                </h3>
                ...
                {(selectedApplication.file_metadata ?? []).map((file) => (
                  <article key={`${file.fieldName}-${file.originalName}`}>
                    <div>
                      <strong>{file.fieldName}</strong>
                      {file.url ? (
              ...
              <section className="score-card">
                <div className="score-heading">
                  ...
                  <strong>
                    {scoreDraft &&
                    scoreCriteria.every(({ key }) => scoreDraft[key] !== null)
                      ? scoreCriteria.reduce(
                          (total, { key }) => total + (scoreDraft[key] ?? 0),
                          0,
                        )
                      : "—"}
                    <small>/ 90</small>
                ...
                <div className="score-fields">
                  {scoreCriteria.map(({ key, label, max }) => (
                    <label key={key}>
                      <span>
                        {label}
                        <small>0–{max}</small>
                ...
                <button
                  className="primary-button score-save"
                  type="button"
                ...
                >
                  {isSavingScore ? "Menyimpan…" : "Simpan penilaian"}
                </button>
              </section>
              <section className="document-list">
